...
export class Block2 implements IBlock{
    ...
    constructor(game : Game, colorIndex: number, color:string){
        ...
        this._topRight = new Position(Math.floor( 1 + Game.COL_SIZE/2), -1);
    }
    ...
    private showHide(){
        ...
        this._block2.draw(this._game.canvas, this._topRight)
    }
    ...
    canMoveDown(): boolean {
        ...
        if (this._angleOfRotation === 90 && (temp.row < Game.ROW_SIZE && this._game.matrix[temp.row][temp.column] === 0 )){
            return true;
        }

        if (this._angleOfRotation === 180 && (temp.row < Game.ROW_SIZE && this._game.matrix[temp.row][temp.column] === 0 ) &&
            (temp1.row < Game.ROW_SIZE && this._game.matrix[temp1.row][temp1.column] === 0 )){
            return true;
        }
    ...
    canMoveRight(): boolean {
        ...
        let temp1 = new Position(this._topLeft.column + 1, this._topLeft.row);

        if (this._angleOfRotation === 90 && temp.column < Game.COL_SIZE && 
            this._game.matrix[temp.row][temp.column] === 0  && this._game.matrix[temp1.row][temp1.column] === 0){
            return true;
        }
    ...
    canMoveLeft(): boolean {
        ...
        let temp1 = new Position(this._topRight.column - 1, this._topRight.row);
    ...
    canRoteate(): boolean {
        ...
        if (angle === 90){
            if (this._topLeft.row > 0 && 
                this._game.matrix[this._topRight.row -1][this._topRight.column -1] === 0 && 
                this._game.matrix[this._topLeft.row -1][this._topLeft.column -1] === 0){
    ...
    rotateCounterClockWise(): boolean {
        if (this.canRoteate()){
            ...
            if (this._angleOfRotation === 90){
                this._topRight = new Position(this._topLeft.column , this._topLeft.row - 1);
            }
            else if (this._angleOfRotation === 180){
                this._topRight = new Position(this._topLeft.column + 1 , this._topLeft.row);
                this._topLeft = new Position(this._topLeft.column, this._topLeft.row);
            }
