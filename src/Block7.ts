...
export class Block7 implements IBlock{
    ...
    private showHide(){
        ...
        this._block3.draw(this._game.canvas, this._bottomLeft);
        this._block4.draw(this._game.canvas, this._topRight);
    }

    private updateMatrix(value: number){
        if (this._angleOfRotation === 0 && this._topLeft.row >=0){
    ...
    canMoveDown(): boolean {
        let temp = new Position(this._bottomLeft.column, this._bottomLeft.row + 1);
    ...
    canMoveRight(): boolean {
        ...
        if (temp.row < Game.ROW_SIZE && temp.column < Game.COL_SIZE && 
    ...
    canMoveLeft(): boolean {
        ...
        let temp1 = new Position(this._bottomLeft.column - 1, this._bottomRight.row);
       
        if (temp.row < Game.ROW_SIZE && temp.column  >= 0 && 
    ...
    drawOnNextItemCanvas(){
        ...
        let position1 = new Position(position.column , position.row + 1);
        let position2 = new Position(position.column + 1, position.row);
