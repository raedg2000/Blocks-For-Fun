...
export class Block8 implements IBlock{
    ...
    canMoveDown(): boolean {
        ...
        let temp1 = new Position(this._topLeft.column, this._topLeft.row + 1);
        let temp2 = new Position(this._bottomRight.column, this._bottomRight.row + 1);
        
        if (this._topLeft.row == -2){
            temp = new Position(this._bottomMiddle.column, this._bottomMiddle.row + 2);
            temp1 = new Position(this._topLeft.column, this._topLeft.row + 2);
            temp2 = new Position(this._bottomRight.column, this._bottomRight.row + 2);
        }
    ...
    canMoveRight(): boolean {
        ...
        let temp2 = new Position(this._bottomMiddle.column + 1, this._bottomMiddle.row);
        let temp3 = new Position(this._topLeft.column + 1, this._topLeft.row);
        
        if (this._angleOfRotation === 0 && temp.row < Game.ROW_SIZE && temp.column < Game.COL_SIZE && 
            this._game.matrix[temp.row][temp.column] === 0 && temp1.column < Game.COL_SIZE && 
            this._game.matrix[temp1.row][temp1.column] === 0){
            return true;
        }

        if (this._angleOfRotation === 90 && temp.row < Game.ROW_SIZE && temp1.column < Game.COL_SIZE && 
    ...
    canMoveLeft(): boolean {
        ...
        if (this._angleOfRotation === 90 && temp.row < Game.ROW_SIZE && temp.column  >= 0 && 
            this._game.matrix[temp.row][temp.column] === 0 && 
            this._game.matrix[temp2.row][temp2.column] === 0 &&
            this._game.matrix[temp3.row][temp3.column] === 0  ){
            return true;
    ...
    canRoteate(): boolean {
        ...
        if (angle === 90){
            if (this._topLeft.row > 0 && this._topLeft.row + 2 < Game.ROW_SIZE &&
                ...
                this._game.matrix[this._bottomRight.row -1][this._bottomRight.column] === 0){
                
                return true;
            }
        }
        
        if (angle === 180 && this._topLeft.row >0 &&
            this._game.matrix[this._topLeft.row][this._topLeft.column + 1] === 0 &&
            this._game.matrix[this._topLeft.row][this._topLeft.column + 2] === 0 &&
            this._game.matrix[this._bottomRight.row][this._bottomRight.column - 1] === 0 &&
    ...
    rotateCounterClockWise(): boolean {
        if (this.canRoteate()){
            ...
            else if (this._angleOfRotation === 180 && this._bottomRight.column + 1 < Game.COL_SIZE){
                this._topLeft = new Position(this._topLeft.column, this._topLeft.row - 2);
                this._topMiddle = new Position(this._topMiddle.column + 1, this._topMiddle.row - 1);
                this._bottomRight = new Position(this._bottomRight.column + 1, this._bottomRight.row + 1);
            }
